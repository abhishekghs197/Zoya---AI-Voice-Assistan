...
export default function App() {
  ...
  const [pendingCommand, setPendingCommand] = useState<{ url: string; action: string } | null>(null);
  ...
  const handleTextCommand = useCallback(async (finalTranscript: string) => {
    ...
    if (commandResult.isBrowserAction && commandResult.url) {
      ...
      setPendingCommand({ url: commandResult.url, action: commandResult.action });
      ...
      if (!win) {
        ...
        // Clear pending command after a delay if it opened successfully
        setTimeout(() => setPendingCommand(null), 5000);
      }
    } else {
    ...
    if (isSessionActive) {
      ...
      try {
        ...
        session.onCommand = (url) => {
          setPendingCommand({ url, action: "Opening requested page..." });
    ...
    <div className="h-[100dvh] w-screen bg-[#050505] text-white flex flex-col items-center justify-between font-sans relative overflow-hidden m-0 p-0">
      ...
      <header className="absolute top-0 left-0 w-full flex justify-between items-center z-20 shrink-0 px-6 py-4 md:px-12 md:py-6">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-3 px-4 py-2 rounded-full border border-cyan-500/30 bg-black/40 backdrop-blur-md shadow-[0_0_15px_rgba(6,182,212,0.1)]">
            <motion.div 
              animate={{ opacity: [1, 0.4, 1] }}
              transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }}
              className="w-2.5 h-2.5 rounded-full bg-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.8)]"
            />
            <span className="text-sm font-bold tracking-[0.2em] text-white uppercase font-mono">
              Zoya Live
            </span>
          </div>
        </div>
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0">
        <Visualizer state={appState}>
          <ZoyaAvatar state={appState} />
        </Visualizer>
      </div>

      {/* Pending Command Fallback */}
      <AnimatePresence>
        {pendingCommand && (
          <motion.div
          ...
          >
            <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-4 flex flex-col items-center gap-3 shadow-2xl">
              <p className="text-sm font-medium text-white/80">{pendingCommand.action}</p>
              <div className="flex gap-2">
                <button
                  onClick={() => {
                    window.open(pendingCommand.url, "_blank");
                    setPendingCommand(null);
                  }}
                  className="px-4 py-2 bg-violet-500 hover:bg-violet-600 rounded-full text-sm font-bold transition-colors shadow-lg"
                >
                  Click to Open
                </button>
                <button
                  onClick={() => setPendingCommand(null)}
                  className="px-4 py-2 bg-white/5 hover:bg-white/10 rounded-full text-sm font-medium transition-colors"
                >
                  Dismiss
                </button>
              </div>
